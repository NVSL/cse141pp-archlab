...
int main(int argc, char *argv[]) {
	...
	archlab_add_si_option<uint64_t>("line-size",  line_size, 64,  "Cache line size");
	...
	array[target * line_size]++;

	for(uint i = 0; i < cache_size/line_size;i++) {
		uint64_t before = rdtsc();
		array[i*line_size]++;
		uint64_t after = rdtsc();
		std::cout << i << " " << after-before << "\n";
	}
